          ...
          <form class="form-horizontal form-label-left" method="post" action="">
            ...
            <div class="form-group">
              ...
              <div class="col-md-9 col-sm-9 col-xs-12">
                <input required="required" type="text" class="form-control" placeholder="Nhập tên loại tin" name="Ten" value="<?= (isset($_GET['idLT']) || $loaitin)? $loaitin['Ten']:'' ?>">
              </div>
            </div>
              ...
              <div class="col-md-9 col-sm-9 col-xs-12">
                <select class="form-control" name="idTL">
                  ...
                  <?php while ($result = mysql_fetch_assoc($theloai)): ?>
                    <option value="<?= $result['idTL'] ?>" selected="select" <?= ($result['idTL'] == $loaitin['idTL'])?'selected="select"':'' ?> > <?= $result['TenTL'] ?> </option>
                  <?php endwhile ?>
                </select>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="col-md-9 col-sm-9 col-xs-12">
                <select class="form-control" name="AnHien">
                  <option value="1" <?= (isset($_GET['idLT']) && $loaitin['AnHien']==1 || $loaitin['AnHien'] ==1 )? 'selected="select"' :'' ?>> Hiện </option>
                  <option value="0" <?= (isset($_GET['idLT']) && $loaitin['AnHien']==0 || $loaitin['AnHien'] ==0 )? 'selected="select"' :'' ?>> Ẩn </option>
                </select>
              </div>
            </div>
...
if(isset($_POST['suaTL'])){
  ...
  $loaitin = $_POST;
// edit_loaitin
  edit_loaitin($idLT,$loaitin);
  
  if(mysql_error()){
    die(mysql_error());
  } else{
    $_SESSION['message'] = "Bạn sửa thành công ".$loaitin['TenTL'];
    header("location:index.php?p=listLoaiTin");
  }
}
 ?>
